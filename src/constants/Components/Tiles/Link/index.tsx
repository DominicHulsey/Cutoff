...
export const LinkTile = (t: {
  ...
  const updateTileDimensions = useCallback(
    async (width: number, height: number) => {
      ...
      try {
        ...
        const detailTileKeys = storageKeys.filter(key =>
          key.startsWith('detail_tiles_'),
        );
  ...
  const handlePress = () => {
    Linking.openURL(t.content).catch(err =>
    ...
    return (
      <TouchableOpacity
        ...
        <View style={styles.linkPreviewLoading} onLayout={onLayoutChange}>
          ...
          <Text style={[styles.linkPreviewDomain, {marginTop: 8}]}>
            Loading preview...
          </Text>
        </View>
      </TouchableOpacity>
